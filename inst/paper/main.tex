...
\begin{equation}\label{eq:general-estimand}
    ...
    \qquad m_j = \E_{\Q\sim\mu}\big[\phi_j(\Delta_S(\Q),\Delta_Y(\Q))\big],
\end{equation}
...
\end{equation}
for $a,b\in\{S,Y\}$, where $\bar a(x)=\E_\mu[w(x)]$. On a finite covariate space the
kernel is a matrix $\Sigma$ and the bilinear term is the quadratic form
$\tau_a^\top\Sigma\,\tau_b$ used above. Consequently the estimand's estimability
...
\begin{itemize}
    ...
    ``elbow'' the minimax rate is slower than $\sqrt{n}$ and no asymptotically
    normal estimator exists
...
(Assumptions A1--A4, A6--A8: identification and CATE asymptotic linearity, bounded
