...
This perspective clarifies that there are no ``true'' types---only varying treatment effects. Discretization schemes that align with treatment effect heterogeneity (e.g., partitioning on estimated $\tau(\bX)$) approximate the underlying continuous heterogeneity more faithfully than arbitrary covariate-based partitions.
...
and correlation broadly agree). DGP~2 has opposite-signed treatment--covariate
interactions ($\gamma_{AX} > 0$, $\beta_{AX} < 0$), yielding strong \emph{negative}
correlation despite moderate PTE. DGP~3 has weak mediation ($\beta_S = 0.3$, PTE
...
The four DGPs already show two regimes in which PTE and the across-study
correlation diverge: DGP~3, where low PTE (0.30) coexists with near-perfect
transportability, and DGP~4, where PTE is undefined ($\Delta_Y(\PP_0) \approx 0$)
yet the correlation is well-defined and near one. A third, sharper illustration
arises with opposite-signed effect modification calibrated so that PTE at $\PP_0$
suggests a good surrogate while the treatment effects are uncorrelated across
studies: a setting with $\mathrm{PTE}(\PP_0) \approx 0.54$ but across-study
$\mathrm{cor}(\Delta_S, \Delta_Y) \approx 0.00$. Mediation analysis would approve
such a surrogate, but its effects do not co-vary across populations, so it fails
to transport. The local geometric evaluation correctly identifies this fragility,
whereas PTE does not.
