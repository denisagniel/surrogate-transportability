...
and share $\mathbb{P}_0$'s conditional law of potential outcomes given $X$. Equivalently, the
reweighting $w=\tfrac{d\mathcal{Q}}{d\mathbb{P}_0}$ is a function of $X$ alone, so
$\tau_S(x),\tau_Y(x)$ are frozen at their $\mathbb{P}_0$ values under every $\mathcal{Q}$,
which is exactly what makes \eqref{eq:delta-linear} hold.
\end{assumption}
...
$n^{-\,4s/(4s+d)}$ (with $s=\tfrac{1}{2}(s_S+s_Y)$ the effective smoothness), strictly slower
...
\ref{assm:geometry}; a genuinely smooth $C$ can only \emph{absorb} smoothness and \emph{relax}
the requirement, so \eqref{eq:elbow} is a conservative sufficient condition here. On finite
...
differentiable with efficient influence function
\begin{equation}
\chi_{ab}(O)
...
$\chi_{aa}(O) = 2\int h_a(x)\,\mathrm{IF}_{\tau_a}(O;x)\,d\mathbb{P}_0(x)-\Theta_{aa}$ with
...
$\mathbb{E}_{\mathbb{P}_0}[\tilde\chi_{ab}(O)g(O)]$ for every score $g$, where $\tilde\chi_{ab}$
is the bracketed term; by the Riesz representation of pathwise derivatives $\tilde\chi_{ab}$ is
the (canonical-gradient) influence function of $\Theta_{ab}$. Since
$\mathbb{E}_{\mathbb{P}_0}[\mathrm{IF}_{\tau_a}(O;x)]=0$ for all $x$ we have
$\mathbb{E}_{\mathbb{P}_0}[\tilde\chi_{ab}]=0$; the constant $-\Theta_{ab}$ in \eqref{eq:chi-ab}
is the centering used for one-step debiasing (whose plug-in part carries $+\Theta_{ab}$).
Efficiency holds because $\tilde\chi_{ab}$ is a $\mathbb{P}_0$-integral of the efficient CATE
influence functions, hence lies in the tangent space.

\emph{Step 5: diagonal check.} If $a=b$ then $h_a=h_b$ and $\mathrm{IF}_{\tau_a}=\mathrm{IF}_{\tau_b}$,
so \eqref{eq:chi-ab} becomes
$\chi_{aa}(O)=2\int h_a(x)\mathrm{IF}_{\tau_a}(O;x)d\mathbb{P}_0(x)-\Theta_{aa}$, the factor $2$
...
$\eta$} holds in addition to orthogonality \emph{in $\beta$}.
...
satisfies $(h_b(k)p_0(k))_k \propto \Sigma\tau_b$, so Lemma~\ref{lem:bilinear-eif-A} gives
$\chi_{ab}(O)=(\Sigma\tau_b)^\top\mathbf{IF}_{\tau_a}(O)+(\Sigma\tau_a)^\top\mathbf{IF}_{\tau_b}(O)-\tau_a^\top\Sigma\tau_b$,
