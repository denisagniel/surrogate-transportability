...
in $\delta$): a genuine kink in $\Theta(p_0)$ exactly at the boundary. This is
...
enough that the TV ball covers the entire probability simplex.
...
\begin{proof}
Throughout, identify $U(\PP_0,\lambda;d_{\mathrm{TV}})$ with the corresponding
subset of the covariate simplex $\Delta_{K-1}$: under A0 the reweighting is a
function of $X$ alone, so $d_{\mathrm{TV}}(\Q,\PP_0)=\tfrac12\sum_k|q_k-p_0(k)|$
exactly, and \eqref{eq:sigma-def}'s $\Sigma=\Cov_\mu(q_k,q_{k'})$ depends only on
the law of $Q=(q_1,\dots,q_K)$ under $\mu$. At saturation,
$\mu=\mathrm{Uniform}(\Delta_{K-1})=\mathrm{Dirichlet}(1,\dots,1)$, which is
...
$\Sigma=aI_K+b\mathbf1\mathbf1^\top$ for scalars $a,b$. Since $\sum_kQ_k=1$ a.s.,
$\Sigma\mathbf1=\Cov(Q,\mathbf1^\top Q)=\Cov(Q,1)=0$, pinning $b=-a/K$ and giving
$\Sigma=a(I_K-\mathbf1\mathbf1^\top/K)$ for some scalar $a>0$ (positive since $Q$ is
non-degenerate for $K\ge2$). This argument uses only exchangeability of $Q$ itself
and the constraint $\mathbf1^\top Q=1$; unlike the interior-regime proof, it makes
no use of the centered variable $V=Q-\PP_0$ --- indeed $V\stackrel{d}{=}\Pi V$ fails
in general here, since $\Pi V=\Pi Q-\Pi\PP_0\stackrel{d}{=}Q-\Pi\PP_0$, which equals
$V$ in distribution only if $\Pi\PP_0=\PP_0$: the feasible set for $V$ at saturation
is $\Delta_{K-1}-\PP_0$, which is not permutation-invariant unless $\PP_0$ itself is
uniform. Substituting $\Sigma=a(I_K-\mathbf1\mathbf1^\top/K)$ into
$\Theta_{ab}=\tau_a^\top\Sigma\tau_b$ and forming the ratio cancels $a$ exactly as in
the proof of Proposition~\ref{prop:interior}, giving
$\Theta(\PP_0,\lambda)=\Corr_K(\tau_S,\tau_Y)$. (The proportionality constant itself
is a separate computation from Dirichlet$(1,\dots,1)$'s moments---
$\mathrm{Var}(Q_k)=(K-1)/(K^2(K+1))$, $\mathrm{Cov}(Q_k,Q_{k'})=-1/(K^2(K+1))$ for
$k\ne k'$, matching $a=1/(K(K+1))$ directly---but is not needed here since $a$
cancels in the ratio regardless of its value, exactly as $a(\lambda)$ does at the
interior end.)
\end{proof}
