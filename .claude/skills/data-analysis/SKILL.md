...
- **Use `readr::write_rds()`** (or `arrow::write_parquet()` for large tabular data) for every computed object — Quarto slides may need them
...
1. `readr::write_rds()` for all key objects (or `arrow::write_parquet()` for large tabular data); regression results, summary tables, processed data
2. Create `output/` subdirectories as needed with `fs::dir_create()`
...
# Inputs: [Data files]
# Outputs: [Figures, tables, RDS/parquet files]
# ============================================================
...
# 5. Export ----
# [readr::write_rds or arrow::write_parquet for all objects, ggsave for all figures]
```
