...
1. **Identify scripts to review:**
   - If `$ARGUMENTS` is a specific `.R` filename or path: review that file only
   - If `$ARGUMENTS` is `all`: review all R scripts in `scripts/`, `scripts/R/`, and `Figures/*/` as applicable
