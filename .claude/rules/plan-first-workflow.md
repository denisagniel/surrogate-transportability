...
8. **Save initial session log and initial session note** — capture goal and key context in `quality_reports/session_logs/` and in `session_notes/YYYY-MM-DD.md` (see session-notes.md)
9. **Implement via orchestrator** — see `orchestrator-protocol.md`

## Step 3: Requirements Specification (For Complex/Ambiguous Tasks)

**When to use:**
- Task is high-level or vague ("improve the slides", "refine the manuscript", "analyze the data")
