#!/usr/bin/env Rscript
# Quick Validation: TV Ball Correlation IF with Slides DGP
#
# QUICK VERSION: 50 replications (~8 minutes)
# Use this for fast iteration and debugging before running the full 500-rep validation
#
...
cat("================================================================\n")
cat("QUICK VALIDATION: TV BALL CORRELATION IF (50 REPS)\n")
cat("================================================================\n\n")
...
saveRDS(
  ...
  "validation/results/tv_ball_IF_slides_dgp_quick.rds"
)

cat("Raw results saved to: validation/results/tv_ball_IF_slides_dgp_quick.rds\n\n")
...
cat("================================================================\n")
cat("QUICK RESULTS SUMMARY (", n_success, " reps)\n", sep = "")
cat("================================================================\n\n")

cat("⚠️  NOTE: This is a QUICK validation with only 50 reps.\n")
cat("   For definitive results, run the full 500-rep validation:\n")
cat("   Rscript validation/validate_tv_ball_IF_slides_dgp.R\n\n")
...
# Coverage (with wider CI due to small sample)
cat("\nCoverage:\n")
...
cat(sprintf("  Coverage rate: %.3f (95%% CI: [%.3f, %.3f])\n",
            ...
            coverage_rate + 1.96 * coverage_se))
cat("  (Wide CI due to small sample; use full validation for precision)\n")
...
cat("\nEstimate Distribution:\n")
cat(sprintf("  Mean ρ̂: %.4f (true: %.4f)\n",
            mean(results_clean$rho_hat), RHO_TRUE))
...
cat("\n================================================================\n")
cat("QUICK CHECK (Preliminary Assessment)\n")
cat("================================================================\n\n")

# Rough criteria (less stringent due to small sample)
bias_ok <- abs(mean(results_clean$bias)) < 0.1  # More slack for 50 reps
coverage_rough <- coverage_rate >= 0.90  # Lower bound less precise
time_ok <- mean(results_clean$time_sec) < 30

cat(sprintf("Bias magnitude: %.4f %s\n",
            abs(mean(results_clean$bias)),
            ifelse(bias_ok, "✓", "⚠️")))

cat(sprintf("Coverage rate: %.1f%% %s\n",
            100 * coverage_rate,
            ifelse(coverage_rough, "✓", "⚠️")))

cat(sprintf("Mean time: %.1f sec %s\n",
            mean(results_clean$time_sec),
            ifelse(time_ok, "✓", "⚠️")))

if (all(c(bias_ok, coverage_rough, time_ok))) {
  cat("\n✓ Quick check suggests method is working correctly\n")
  cat("  Run full validation for definitive assessment\n")
} else {
  cat("\n⚠️  Potential issues detected\n")
  cat("  Investigate before running full validation\n")
}

cat("\n================================================================\n")
cat("Next step: Rscript validation/validate_tv_ball_IF_slides_dgp.R\n")
cat("================================================================\n")
