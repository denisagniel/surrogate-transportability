...
**Assumptions A0–A8** (ledger with what each buys / how to relax): **A0 conditional-effect transportability given X** (NEW, substantive causal assumption — cross-study effect variation is compositional in X; effects frozen within X; distinct from A3 support), A1 identification, A2 CATE asymptotic-linearity (load-bearing; only its IF is needed downstream), A3 Q≪P₀ + bounded weights, A4 convex geometry + geometric-ergodic hit-and-run, **A5 quadratic/bilinear-functional √n-estimability**, A6 cross-fit nuisance rates, A7 smooth non-degenerate Ψ, A8 n & M regime.

**TWO boundaries:** A0 (causal/within-X — governs what the estimand MEANS) and A5 (statistical/smoothness — governs how well we ESTIMATE it). Neither removable; each has an honest fallback.

**A0 relaxation → observation-level partial-ID interval (§3bis, verified in `explorations/obs_level/`):** obs-level estimand perturbs U|X not just X; equals X-level iff A0 holds, else more conservative (the gap = exposure to unmeasured within-X heterogeneity, real signal). Within-X effect VARIANCES identified (arm-variance differencing); within-X CROSS-COVARIANCE not identified (PO-correlation non-ID — verified the naive estimator breaks under effect-baseline dependence). ⇒ sweep cross-cov over its Cauchy-Schwarz range → [ρ_lo,ρ_hi] = data-powered A0-sensitivity bounds. The old observation_level_minimax code did NOT do this (reweighted by X-distance; identical-X units identical weights — X-level in disguise).
