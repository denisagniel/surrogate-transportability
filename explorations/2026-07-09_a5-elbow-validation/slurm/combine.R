...
# writes the ONLY home-directory artifact of the run: output/<run-id>.rds.
#
...
CODE_FILES <- c("config/grid.R", "R/dgp_smooth.R", "R/pseudo_outcome.R",
                "R/bilinear_estimator.R", "R/run_one_stage1.R")
...
if (file.exists(recorded_hash_file)) {
  ...
  if (!identical(recorded, current_hash)) {
    stop(sprintf(paste0(
      ...
      "source files now hash to %s.\nThe simulation code changed after this run ",
      "started. Re-profile and re-submit, or clean this run-id with clean.sh."),
...
result <- result[order(result$unit, result$estimator), , drop = FALSE]
n_units_expected <- nrow(unit_table_stage1(REPS_STAGE1))
cat(sprintf("Combined %d rows (expected ~%d units x 2 estimators).\n",
            nrow(result), n_units_expected))
...
results_dir <- file.path(opt$study_dir, "output")
