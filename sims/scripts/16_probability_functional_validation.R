...
#' Probability Functional Validation
#'
#' Research Questions:
#' 1. Is φ̂_probability unbiased for true φ_probability?
#' 2. Do 95% CIs achieve nominal coverage?
#' 3. How does performance vary with thresholds (ε_S, ε_Y)?
#'
#' Functional Definition:
#'   φ_prob(F_λ; ε_S, ε_Y) = P(ΔY > ε_Y | ΔS > ε_S)
#'
#' This tests whether the probability functional correctly estimates
#' the conditional probability of meaningful outcome effects given
#' meaningful surrogate effects.
...
cat("================================================================\n")
cat("PROBABILITY FUNCTIONAL VALIDATION\n")
cat("================================================================\n\n")

cat("Research Questions:\n")
cat("  1. Is φ̂_probability unbiased for true φ_probability?\n")
cat("  2. Do 95% CIs achieve nominal coverage?\n")
cat("  3. How does performance vary with thresholds?\n\n")

cat("Functional Definition:\n")
cat("  φ_prob(F_λ; ε_S, ε_Y) = P(ΔY > ε_Y | ΔS > ε_S)\n")
...
for (scenario_name in names(scenarios)) {
  ...
  for (thresh_name in names(threshold_scenarios)) {
    ...
    for (rep in 1:N_REPLICATIONS) {
      ...
      # Step 2: Compute TRUE φ_probability(F_λ) using reweighting
      ...
      # TRUE probability: P(ΔY > ε_Y | ΔS > ε_S)
      ...
      true_probability <- sum(true_effects[, 1] > thresh$epsilon_s &
      ...
      covered <- (true_probability >= method_result$ci_lower) &&
                 (true_probability <= method_result$ci_upper)
      ...
      if (iteration %% 100 == 0) {
        ...
        saveRDS(validation_results,
                sprintf("sims/results/probability_validation_interim_iter%04d.rds", iteration))
      }
    }
    cat("\n")
  }
...
coverage_summary <- validation_results %>%
  ...
  summarise(
    ...
    mean_bias = mean(method_estimate - true_probability, na.rm = TRUE),
...
overall_bias <- mean(validation_results$method_estimate -
                     validation_results$true_probability, na.rm = TRUE)
...
p1 <- ggplot(coverage_summary, aes(x = lambda, y = coverage_rate,
  ...
  ylim(0.88, 1.0) +
  labs(
    title = "Probability Functional: Coverage Rate vs. Lambda and Thresholds",
...
ggsave("sims/results/probability_coverage.png", p1,
       width = 10, height = 6, dpi = 300)

cat("  Saved: sims/results/probability_coverage.png\n")

# Plot 2: True probability vs. Method estimate
p2 <- ggplot(validation_results,
             aes(x = true_probability, y = method_estimate,
                 color = threshold_scenario)) +
  geom_point(alpha = 0.3, size = 1) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_grid(threshold_scenario ~ scenario) +
  labs(
    title = "True φ_prob(F_λ) vs. Method Estimate",
    subtitle = "Points should cluster tightly around diagonal",
    x = "True Probability (from reweighting approach)",
    y = "Method Estimate"
  ) +
  theme_minimal(base_size = 10) +
  theme(legend.position = "none")

ggsave("sims/results/probability_calibration.png", p2,
       width = 12, height = 10, dpi = 300)

cat("  Saved: sims/results/probability_calibration.png\n")

# Plot 3: CI coverage visualization (sample)
validation_results_plot <- validation_results %>%
  arrange(scenario, threshold_scenario, true_probability) %>%
...
p3 <- ggplot(validation_results_plot,
  ...
  geom_pointrange(aes(ymin = method_ci_lower, ymax = method_ci_upper,
                      color = covered),
                  alpha = 0.6, size = 0.3) +
  geom_point(aes(y = true_probability), color = "black", size = 1) +
  facet_wrap(~threshold_scenario, scales = "free_x") +
  scale_color_manual(values = c("TRUE" = "blue", "FALSE" = "red"),
                     labels = c("TRUE" = "Covered", "FALSE" = "Missed")) +
  labs(
    title = "Probability Functional: CI Coverage (λ=0.3)",
    subtitle = "Black dots: true φ_prob(F_λ); Blue/Red: CIs that cover/miss",
...
ggsave("sims/results/probability_ci_coverage.png", p3,
       width = 12, height = 8, dpi = 300)

cat("  Saved: sims/results/probability_ci_coverage.png\n")
...
saveRDS(validation_results, "sims/results/probability_validation_detailed.rds")
cat("  Saved: sims/results/probability_validation_detailed.rds\n")

saveRDS(coverage_summary, "sims/results/probability_validation_summary.rds")
cat("  Saved: sims/results/probability_validation_summary.rds\n")

# Save as CSV
write.csv(coverage_summary,
          "sims/results/probability_validation_summary.csv",
          row.names = FALSE)
cat("  Saved: sims/results/probability_validation_summary.csv\n")
...
by_threshold <- coverage_summary %>%
  ...
  summarise(mean_coverage = mean(coverage_rate),
            mean_probability = mean(mean_true_probability),
            .groups = "drop")

for (i in 1:nrow(by_threshold)) {
  row <- by_threshold[i, ]
  cat(sprintf("   %s (ε_S=%.2f, ε_Y=%.2f): coverage=%.1f%%, mean prob=%.3f\n",
              row$threshold_scenario, row$epsilon_s, row$epsilon_y,
              row$mean_coverage * 100, row$mean_probability))
}

cat("\n3. Calibration:\n")
cat(sprintf("   Overall bias: %.4f (%.2f%% relative)\n",
            overall_bias,
            overall_bias / mean(validation_results$true_probability) * 100))
cat(sprintf("   SE/SD ratio: %.2f (1.0 = perfectly calibrated)\n",
            mean(coverage_summary$se_sd_ratio)))

cat("\n4. Paper Claims:\n")
if (overall_coverage >= CONFIDENCE_LEVEL - 0.02) {
  cat("   ✓ Probability functional achieves nominal 95% coverage\n")
  cat("   ✓ Method correctly estimates P(ΔY > ε_Y | ΔS > ε_S)\n")
  cat("   ✓ Robust to threshold choice\n")
} else {
  cat("   ⚠ Coverage deviates from nominal level\n")
  cat("   → Check: Is M large enough? Thresholds too extreme?\n")
}

cat("\n5. Recommendation:\n")
if (overall_coverage >= CONFIDENCE_LEVEL - 0.02) {
  cat("   Add to Section 5 (Simulation Studies):\n")
  cat("   'The probability functional φ_prob(F_λ; ε_S, ε_Y) = P(ΔY > ε_Y | ΔS > ε_S)\n")
  cat("    measures the conditional probability of meaningful outcome effects.\n")
  cat(sprintf("    Across λ ∈ [0.1, 0.5] and four threshold scenarios, coverage was %.1f%%\n",
              overall_coverage * 100))
  cat("    (target: 95%), validating the inference procedure.'\n")
} else {
...
cat("\n")
cat("================================================================\n")
cat("Probability functional validation complete!\n")
cat("================================================================\n")
