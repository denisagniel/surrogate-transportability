...
- Repeatedly explaining the same 3+ step workflow to the agent
...
The `description` field determines when the agent loads your skill. Use this structure:
...
**Solution:** Check output/ directory, verify readr::write_rds() (or parquet) was called
...
3. Restart your agent or run `/reload` if available
